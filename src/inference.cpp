...
ModelInference::ModelInference(const std::string& model_path, int batch_size)
...
ModelInference::ModelInference() : batch_size(0), device(torch::kCPU) {
        // This creates an uninitialized model that won't be used
}

// Convert a single StateNode to tensor inputs
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> ModelInference::state_to_tensors(const StateNode* node) {
	// Pawn state - 2x9x9
...
void ModelInference::queue_for_evaluation(StateNode* node) {
...
void ModelInference::flush_queue() {
...
void ModelInference::process_batch() {
...
float ModelInference::evaluate_node(StateNode* node) {
	auto [pawn_tensor, wall_tensor, meta_tensor] = state_to_tensors(node);
