...
//enable shared from this so I can make shared pointers to set child->parent references
class StateNode : public std::enable_shared_from_this<StateNode>
{
public:
	...
	StateNode(shared_ptr<StateNode> parent, Move move, int score); //generate a child node, with a reference to the parent and a the move that takes gamestate from parent->child
	...
	shared_ptr<StateNode> get_best_move();
	...
	shared_ptr<StateNode> play_out(); //simulates by random choice to determine a winner from this state
...
bool test_and_add_move( shared_ptr<StateNode> state, Move move); //helps testing validity and also beginning scoring before state fully initialized.
